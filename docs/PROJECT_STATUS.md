...
1. ~~**Mentions légales, CGU, politique de données**~~ ✅ livré 2026-04-26 — modèle `LegalPage` (slug + content markdown + version, seedé avec un brouillon initial conforme loi 18-07 DZ), pages publiques `/mentions-legales`, `/cgu`, `/confidentialite` qui rendent depuis la BDD via un mini-renderer markdown (`src/lib/simple-markdown.ts`, sans dépendance externe). Édition admin sur `/admin/legal` avec aperçu live + versioning auto. Bandeau cookies (composant client + localStorage `immodz-cookies-ack`). Checkbox d'acceptation CGU + politique de confidentialité bloquante à l'inscription. Footer mis à jour avec les vrais liens.
2. ~~**Signalement d'annonce**~~ ✅ livré 2026-04-26 — modèle `ListingReport` (7 motifs : `FAKE`, `PRICE`, `ALREADY_RENTED`, `DUPLICATE`, `INAPPROPRIATE`, `WRONG_LOCATION`, `OTHER`), bouton "Signaler" sur la fiche annonce avec modal + commentaire libre, rate limit 5/h/user, anti-doublon (signalement déjà PENDING du même user). File d'attente `/admin/reports` groupée par annonce avec actions : marquer traité, rejeter le signalement, ou rejeter l'annonce (passe `Listing.status` à `REJECTED` avec motif et auto-clôture les autres signalements PENDING de la même annonce). Tuile dans `/admin` (rouge si > 0).
